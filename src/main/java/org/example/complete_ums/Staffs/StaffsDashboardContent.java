...
public class StaffsDashboardContent implements Initializable {
    ...
    private Label totalActiveAccountants, LastLoginOn, YourDesignation, TotalStudents,
            TotalStaff, errorMessageLabel, AttendancePercent, TotalAccountants,
    ...
    public StaffsDashboardContent() throws SQLException {
    }

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        getLastLoginAttempts();
        getTotalStaffssActive();
    ...
    private void getTotalStaffssActive() {
        String query = "SELECT COUNT(*) FROM Users WHERE Role = 'Staff' AND User_Status = " +
                "'Active'";
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            alertManager.showAlert(Alert.AlertType.ERROR, "Database Error", "Admin Says",
                    "Something weird happened while counting total Active Staffs: " + e.getMessage());
        }
    }

    private void setDepartmentAndCourses() {

        String query = "select * from Staffs where User_Id=" + userIdLoggedIn;
